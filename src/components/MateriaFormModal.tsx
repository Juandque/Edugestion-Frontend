...
import {type Materia, type MateriaRequest} from '../dtos/Materia'
import toast from 'react-hot-toast';

interface Props {
    onClose: () => void;
    onSuccess: () => void;
    materiaParaEditar?: Materia | null;
}

const MateriaFormModal: React.FC<Props> = ({ onClose, onSuccess, materiaParaEditar }) => {
    const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<MateriaRequest>();

    useEffect(()=> {
        if(materiaParaEditar){
            reset(materiaParaEditar);
        }
    }, [materiaParaEditar, reset]);

    const onSubmit = async (data: MateriaRequest) => {
        try {
            if(materiaParaEditar){
                await api.patch(`/materias/${materiaParaEditar.id}`, data);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="flex justify-between items-center p-6 border-b border-gray-100">
                            <h3 className="text-xl font-bold text-gray-800">{materiaParaEditar ? 'Editar Materia' : 'Registrar Materia'}</h3>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    {...register("creditos", { 
                                        required: "El numero de creditos es obligatorio", 
                                        valueAsNumber: true, 
                                        min: {
                                            value: 1,
                                            message: "El valor minimo debe ser 1"
                                        }
                                    })}
                                    className="w-full px-4 py-2 rounded-lg border border-gray-200 focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                                    placeholder="Ej: 4"
